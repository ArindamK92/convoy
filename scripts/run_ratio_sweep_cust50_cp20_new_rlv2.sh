...
RL_V2_EXTRA="--print-solution --save-model --checkpoint-dir ${RL_V2_CHECKPOINT_DIR} --seed ${SEED} --epochs ${EPOCHS} --fixed-eval-every ${FIXED_EVAL_EVERY} --rl-algo pomo --baseline shared --pomo-num-starts 10 --pomo-num-augment 8 --decode-type beam_search --decode-beam-width 10"
...
for ratio in $(seq "${RATIO_START}" "${RATIO_END}"); do
  ...
  results_file="ratio_sweep_rlv2_c${TOTAL_CUSTOMERS}_cp${CP_NUM}_r${ratio}_ev${ev_num}_seed${SEED}_e${EPOCHS}.csv"
