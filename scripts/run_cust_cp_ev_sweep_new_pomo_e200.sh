...
#
# Flow:
# 1) Pretrain + save convoy_hybrid checkpoint (POMO + beam-search).
# 2) Pretrain + save convoy_rl_partial_ch checkpoint (POMO + beam-search).
# 3) Run customer sweep via convoy_main using saved checkpoints (POMO enabled).
...
HYBRID_CHECKPOINT_DIR="${HYBRID_CHECKPOINT_DIR:-checkpoints_vrptw/hybrid_c50_cp10_ev10_e200_pomo}"
RL_CHECKPOINT_DIR="${RL_CHECKPOINT_DIR:-checkpoints_vrptw/rl_partial_c50_cp10_ev10_e200_pomo}"
...
# POMO + beam-search settings.
RL_EXTRA="--print-solution --save-model --seed ${SEED} --epochs ${EPOCHS} --fixed-eval-every ${FIXED_EVAL_EVERY} --rl-algo pomo --baseline shared --pomo-num-starts 10 --pomo-num-augment 8 --decode-type beam_search --decode-beam-width 10"
...
echo
echo "=== Step 1: Pretrain + save convoy_hybrid checkpoint (POMO + beam-search) ==="
hybrid_best_ckpt="${HYBRID_CHECKPOINT_DIR}/best_model_hybrid_pomo.ckpt"
...
echo
echo "=== Step 2: Pretrain + save convoy_rl_partial_ch checkpoint (POMO + beam-search) ==="
rl_best_ckpt="${RL_CHECKPOINT_DIR}/best_model_pomo.ckpt"
...
echo
echo "=== Step 3: Sweep using saved checkpoints (POMO + beam-search) ==="
for cust_num in $(seq "${CUST_START}" "${CUST_STEP}" "${CUST_END}"); do
  ...
  results_file="result_pomo_seed_${SEED}_${SEED}_cust${cust_num}_e${EPOCHS}.csv"
