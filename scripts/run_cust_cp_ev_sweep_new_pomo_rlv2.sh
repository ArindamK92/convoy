...
#
# Flow:
# 1) Pretrain + save convoy_rl_partial_ch2 checkpoint (POMO + beam-search).
# 2) Run customer sweep via convoy_main (--only-rl-v2) using saved checkpoint.
...
# POMO + beam-search settings used for both pretraining and sweep-time inference.
RL_V2_EXTRA="--print-solution --save-model --seed ${SEED} --epochs ${EPOCHS} --fixed-eval-every ${FIXED_EVAL_EVERY} --rl-algo pomo --baseline shared --pomo-num-starts 10 --pomo-num-augment 8 --decode-type beam_search --decode-beam-width 10"
...
echo
echo "=== Step 2: Sweep using saved RL-v2 checkpoint (POMO + beam-search) ==="
for cust_num in $(seq "${CUST_START}" "${CUST_STEP}" "${CUST_END}"); do
  ...
  results_file="result_rlv2_pomo_seed_${SEED}_${SEED}_cust${cust_num}_e${EPOCHS}.csv"
