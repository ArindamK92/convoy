...
from convoy_hybrid_v2.convoy_hybrid_kdtree import create_kd_tree, find_nearest_cp
...
def _build_node_to_nearest_cp_map(
    node_info: dict[int, NodeInfo], kd_tree
) -> dict[int, int]:
    """Precompute nearest CP for every known node (depot/customers/CPs)."""
    out: dict[int, int] = {}
    for nid, ninfo in node_info.items():
        nearest_cp = find_nearest_cp(kd_tree, ninfo.x, ninfo.y)
        out[int(nid)] = int(nearest_cp["cp_id"])
    return out


def _can_reach_any_cp_from_node(
    *,
    node_id: int,
    soc_kwh: float,
    cp_ids: list[int],
    ...
    """Return True if node can reach at least one CP with available SoC."""
    eps = 1e-9
    for cp_id in cp_ids:
        required = max(0.0, _get_matrix_value(dist_full, node_id, cp_id) * energy_rate)
        if required <= (soc_kwh + eps):
            return True
    return False
...
def augment_path_with_nearest_cp(
    ...
    """Insert nearest CP when needed by SoC feasibility/safety before next target."""
    ...
    node_to_nearest_cp = _build_node_to_nearest_cp_map(node_info, kd_tree)
    customer_to_nearest_cp: dict[int, int] = {
        int(nid): int(cp_id)
        for nid, cp_id in node_to_nearest_cp.items()
        if node_info.get(int(nid)) is not None
        and node_info[int(nid)].node_type == "customer"
    }
    ...
    for route_idx, route in enumerate(routes_global, start=1):
        ...
        for target_id in route[1:]:
            ...
            direct_reachable = direct_energy <= (current_soc + eps)
            must_recharge_before_target = False
            if direct_reachable and target_info.node_type == "customer":
                # Safety check: do not visit customer if post-visit SoC cannot reach any CP.
                soc_after_direct = max(current_soc - direct_energy, 0.0)
                can_reach_cp_after_customer = _can_reach_any_cp_from_node(
                    node_id=target_id,
                    soc_kwh=soc_after_direct,
                    cp_ids=cp_ids,
                    dist_full=dist_full,
                    energy_rate=energy_rate,
                )
                if not can_reach_cp_after_customer:
                    must_recharge_before_target = True

            if direct_reachable and not must_recharge_before_target:
            ...
            # Need a recharge stop before target.
            cp_inserted = False
            cp_id = int(node_to_nearest_cp.get(current_id, cp_ids[0]))

            energy_to_cp = max(0.0, _get_matrix_value(dist_full, current_id, cp_id) * energy_rate)

            # If already at CP, consider battery recharged to full before next leg decision.
            if (
                cp_id == current_id
                and node_info.get(current_id) is not None
                and node_info[current_id].node_type == "cp"
            ):
                current_soc = effective_battery_kwh
                cp_inserted = True
                energy_to_cp = 0.0

            # If nearest CP is unreachable, try any reachable CP from current node.
            if energy_to_cp > (current_soc + eps):
                reachable_cps = []
                for cpid in cp_ids:
                    e_req = max(0.0, _get_matrix_value(dist_full, current_id, cpid) * energy_rate)
                    if e_req <= (current_soc + eps):
            ...
            if cp_id != current_id and energy_to_cp <= (current_soc + eps):
                current_soc, current_time, step = _append_leg(
                    trace,
                    route_aug,
                    node_info,
    ...
    return {
        "node_to_nearest_cp": node_to_nearest_cp,
