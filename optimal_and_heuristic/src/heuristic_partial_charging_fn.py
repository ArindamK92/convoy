...
from utility import create_kd_tree, find_nearest_cp
...
def heuristic(cp, deliveries, theta, reward, C, D, E, tau_start, tau_end, nS, EVs, gamma_DD, psi_DD, gamma_DC, psi_DC, beta_f, rateE, rateC):
    ...
    for d in deliveries:
        nearest_cp_index = find_nearest_cp(kd_tree, d.latitude, d.longitude, min_lat, max_lat, min_lon, max_lon, min_theta, max_theta, denom_lat, denom_lon, denom_theta)
        d.nearest_CP_local_id = nearest_cp_index
                ...
                if energy_req < energy_cost_min:
                    ...
                    if energy_req + energy_req_d2W > e.B_res:
                        if last_loc.type == "delivery": # if already at a CP then charge is already full
                            ...
                            e.visited_CP_sequence.append(cp[nearest_cp_id_from_last_point])
                            
                            # Compute energy requirement
                            energy_to_CP = psi_DC[(last_loc.local_id, nearest_cp_id_from_last_point)]
                            e.subtrip_energy = e.subtrip_energy + energy_to_CP
    ...
    for e in EVs:
        ...
        if last_loc.type == "delivery":
            ...
            e.visited_CP_sequence.append(cp[0])
            energy_to_W = psi_DC[(last_loc.local_id, cp[0].local_id)]
            e.subtrip_energy = e.subtrip_energy + energy_to_W
            e.subtrips_energy_sequence.append(e.subtrip_energy)
        if last_loc.type == "cp":
            if last_loc.local_id != 0:
                ...
                VC = e.visited_CP_sequence[:-1]
                VC.append(cp[0]) # Add the depot as last charging point
                e.visited_CP_sequence = VC
                
                last_subtrip_energy = e.subtrips_energy_sequence[-1] - psi_DC[(e.A[-2].local_id, last_loc.local_id)] + psi_DC[(e.A[-2].local_id, cp[0].local_id)]
                SES = e.subtrips_energy_sequence[:-1]
                SES.append(last_subtrip_energy)
                e.subtrips_energy_sequence = SES
    ...
    # Step 3: Find partial charging
    partial_ch_per_ev = {}
    partial_ch_cost_per_ev = {}
    total_cost = 0
    total_energy = 0
    
    for e in EVs:
        partial_charging_sequence = [0]
        partial_charging_cost_sequence = [0]
        # print("EV: ", e.local_id, "\n Visited CP:")
        ...
        for l in range(1, L):
            ...
            SoC = SoC - energy_used_in_last_subtrip
            unit_cost_current_cp = theta[cp_current.local_id]
            energy_to_next_best_cp = 0
            ...
            for l2 in range(l+1, L):
                ...
                if unit_cost_at_l2_cp < unit_cost_current_cp:
                    ...
                    total_energy = total_energy + charging_amount
                    break
        partial_ch_per_ev[e.local_id] = partial_charging_sequence
        partial_ch_cost_per_ev[e.local_id] = partial_charging_cost_sequence
        
        
    # print("Partial Charging:\n", partial_ch_per_ev)
    # print("Partial Charging cost:\n", partial_ch_cost_per_ev)
    ...
    for e in EVs:
        ...
        for loc in A[1:]:
            ...
            if loc.type == "delivery":
                delivery_done += 1
                total_reward = total_reward + reward[loc.local_id]
