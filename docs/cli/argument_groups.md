...
| --- | --- | --- | --- |
| Combined runner wrapper args | `convoy_main.py` | `--only-rl`, `--only-opt-heu`, `--only-rl-v2`, `--rl-v2-checkpoint-dir`, `--hybrid-checkpoint-dir`, `--opt-rl-extra`, `--opt-heu-extra`, `--iterations`, `--results-file`, `--clear-rl-checkpoints` | `README.md` (repo root), `docs/cli/convoy_main.md` |
...
| Convoy RL partial charging args | `tests/test_convoy_CPs1_ch2.py` -> `src/convoy_rl_partial_ch2/convoy_rl_main.py` | RL algo/decoder/train/checkpoint flags, EV/cost/reward flags | `src/convoy_rl_partial_ch2/README.md`, `tests/README.md`, `docs/cli/test_convoy_CPs1_ch2.md` |
| Convoy hybrid args | `tests/test_m_VRPTW.py` -> `m_VRPTW/convoy_hybrid_main.py` | RL algo/decoder/train/checkpoint flags, `--verbose`, fixed-instance flags | `m_VRPTW/README.md`, `tests/README.md`, `docs/cli/test_convoy_hybrid.md` |
...
- Stage-specific checkpoint overrides:
  - `--hybrid-checkpoint-dir` for `m_VRPTW`
  - `--rl-v2-checkpoint-dir` for `convoy_rl_partial_ch2`
